...
public final class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    private synchronized void connectInt(LocalBluetoothProfile profile) {
        if (!ensurePaired()) {
            return;
        }
    ...
    public void unpair() {
        ...
        if (state == BluetoothDevice.BOND_BONDING) {// 正在配对则先取消配对
        ...
        if (state != BluetoothDevice.BOND_NONE) {
            ...
            if (dev != null) {
                final boolean successful = dev.removeBond();// 解除或者取消配对
                if (successful) {
                    if (Utils.D) {
